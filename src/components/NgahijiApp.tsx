...
export default function NgahijiApp({ communities, events, stories, catalogError, contentError, liveFeed }: Props) {
  ...
  function submitRegistration(formData: FormData, event: CatalogEvent) {
    ...
    setModal({ type: 'payment', event, names });
  }
    ...
    <>
      ...
      <header className="top">
        <div className="wrap nav">
          ...
          <div className="navright"><button className="iconbtn" aria-label="Cari event dan cerita" onClick={() => setModal({ type: 'search' })}><Search /></button><button className="btn light desktop" onClick={() => setModal({ type: 'auth', mode: 'login' })}>Masuk</button><button className="btn" onClick={() => setModal({ type: 'auth', mode: 'join' })}>Join Ngahiji <span>↗</span></button></div>
        </div>
      </header>
      <main id="main">
        ...
        <section className="wrap closing" id="join"><div className="eyebrow">Your next chapter starts here</div><h2>LET'S NGAHIJI.</h2><p>Temukan event, cerita dan orang-orang yang membuat perjalananmu lebih berarti.</p><button className="btn" onClick={() => setModal({ type: 'auth', mode: 'join' })}>JOIN NGAHIJI <span>↗</span></button></section>
      </main>
...
function RegistrationModal({ event, quantity, setQuantity, onSubmit }: { event: CatalogEvent; quantity: number; setQuantity: (value: number) => void; onSubmit: (formData: FormData) => void }) {
  const ticket = lowestTicket(event);
  return <><div className="eyebrow">Registration / Simulasi tiket</div><h2>Good things start with a hello.</h2><p>{event.title} · {quantity} peserta · {money((ticket?.price_idr ?? 0) * quantity)}</p><form action={onSubmit} className="registration-form"><label htmlFor="quantity">Jumlah peserta (maks. 5)</label><input id="quantity" type="number" min={1} max={5} value={quantity} onChange={(event) => setQuantity(Number(event.target.value))} /> <h3>Data pembeli</h3><label htmlFor="buyerName">Nama lengkap</label><input id="buyerName" name="buyerName" required maxLength={90} defaultValue="" /><label htmlFor="buyerEmail">Email</label><input id="buyerEmail" name="buyerEmail" type="email" required defaultValue="" /><h3>Data peserta</h3>{Array.from({ length: quantity }, (_, index) => <fieldset key={index}><legend>Peserta {index + 1}</legend><label htmlFor={`attendee-${index}`}>Nama lengkap</label><input id={`attendee-${index}`} name={`attendee-${index}`} required defaultValue="" /><label htmlFor={`attendee-email-${index}`}>Email peserta</label><input id={`attendee-email-${index}`} name={`attendee-email-${index}`} type="email" required defaultValue="" /></fieldset>)}<p className="notice">Alur tiket, pembayaran, dan QR masih simulasi. Tidak ada transaksi nyata yang dibuat.</p><label className="checkrow"><input type="checkbox" required /> Saya memahami ini simulasi tiket, bukan pembelian atau registrasi nyata.</label><button className="btn" type="submit">Tinjau registrasi ↗</button></form></>;
}
