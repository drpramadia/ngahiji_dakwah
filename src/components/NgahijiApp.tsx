  ...
  return (
    <>
      ...
      <main id="main">
        ...
        <section className="wrap closing" id="join"><div className="eyebrow">Your next chapter starts here</div><h2>LET&apos;S NGAHIJI.</h2><p>Temukan event, cerita dan orang-orang yang membuat perjalananmu lebih berarti.</p>{viewer ? <a className="btn" href={viewer.homeHref}>Lanjut ke {viewer.role === 'ADMIN' ? 'Admin' : viewer.role === 'ORGANIZER' ? 'Organizer' : 'Member Area'} <span>↗</span></a> : <button className="btn" onClick={() => setModal({ type: 'auth', mode: 'join' })}>JOIN NGAHIJI <span>↗</span></button>}</section>
      </main>
      ...
      <nav className="dock" aria-label="Navigasi seluler"><a className="active" href="#home"><Home />Home</a><a href="#events"><CalendarDays />Events</a><a href="#live"><Play />Live</a><a href="#media"><Newspaper />Media</a>{viewer ? <a href={viewer.homeHref}><UserRound />Profile</a> : <button onClick={() => setModal({ type: 'auth', mode: 'login' })}><UserRound />Profile</button>}</nav>
